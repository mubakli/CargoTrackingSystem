...
def build_tree():
    ...
    # Ekstra yollar ekleme (her şehir için iki alternatif rota sağlama)
                ...
                for other_level in levels.values():
                    for potential_child in other_level:
                        if potential_child != city and all(potential_child != child[0] for child in city.children):
                            city.add_child(potential_child, cost=10)  # Varsayılan ekstra maliyet
                            if len(city.children) >= 2:
                                break
                    if len(city.children) >= 2:
                        break
...
def print_tree(node, level=0, visited=None):
    ...
    if node.city_id in visited:
        print("  " * level + f"{node.city_name} (ID: {node.city_id}) - [Cyclic Reference]")
        return
...
def calculate_depth(node, visited=None):
    ...
    if node.city_id in visited:
        return 0  # If already visited, don't count further depth.
    ...
    return 1 + max(calculate_depth(child, visited) for child, _ in node.children)
...
def list_routes_to_city(node, target_city_name, current_path=None, current_cost=None, routes=None, visited=None):
    ...
    if node.city_id in visited:
        return routes  # Avoid revisiting nodes.
